...
async def main(lidar: RPLidar):
    ...
    async with asyncio.TaskGroup() as tg:
        tg.create_task(wait_and_stop(15, lidar.stop_event))
...
if __name__ == "__main__":
    ...
    filtered = [(angle, dist) for angle, dist in lidar.output_dict.items() if dist is not None]

    if not filtered:
        print("No valid scan data collected.")
    else:
        angles, distances = zip(*filtered)
